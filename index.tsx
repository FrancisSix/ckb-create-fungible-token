...
function IssuedToken({ onIssued }: { onIssued?: (args: string) => void }) {
        ...
        onClick={() =>
          issueToken(privKey, amount)
            .then((result) => {
              ...
              if (onIssued && result.targetOutput.type?.args) {
                onIssued(result.targetOutput.type.args);
              }
            })
            .catch(alert)
        }
...
function ViewIssuedToken({ initialArgs }: { initialArgs?: string }) {
  const [xudtArgs, setXudtArgs] = useState<string>(initialArgs || '');
  const [cells, setCells] = useState<ccc.Cell[]>([]);

  useEffect(() => {
    if (initialArgs) {
      setXudtArgs(initialArgs);
    }
  }, [initialArgs]);
  ...
  return (
    <>
      ...
      <div>
        ...
        <input id="xudt-args" type="text" style={{width: '80%'}} value={xudtArgs} onChange={(e) => setXudtArgs(e.target.value)} />
      </div>
...
export function App() {
  const [issuedXudtArgs, setIssuedXudtArgs] = useState<string>();
  return (
    <div>
      <h1>
      ...
      <IssuedToken onIssued={(args) => setIssuedXudtArgs(args)} />
      <hr />
      <ViewIssuedToken initialArgs={issuedXudtArgs} />
      <hr />
      <TransferIssuedToken />
    </div>
  );
